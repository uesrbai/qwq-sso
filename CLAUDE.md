...
1. ✅ **2FA（TOTP）：v3.3.6 已实现**（Passkey 仍待做，见文末章节）。`server/twofa.js` 用 Node
   ...
   ⚠️ **二维码没做**：绑定页只给密钥手动录入 + otpauth 链接（手机点链接可直接打开 App）。
   没做是因为不能把 TOTP 密钥发给任何第三方二维码服务（泄露），而内嵌 QR 库体积大，暂缓。
