...
// ── 用户端：兑换商品 ──
router.post('/shop/exchange/:id', requireAuth, (req, res) => {
  ...
  db.transaction(() => {
    ...
    db.prepare('INSERT INTO shop_records (id,user_id,user_name,uid_seq,goods_id,goods_name,goods_icon,cost,status) VALUES (?,?,?,?,?,?,?,?,?)')
      .run(uuidv4(), user.id, user.name, user.uid_seq, goods.id, goods.name, goods.icon, goods.cost, 'pending');
    if (goods.stock > 0) {
      db.prepare("UPDATE shop_goods SET stock=stock-1,exchange_count=exchange_count+1,updated_at=datetime('now') WHERE id=?").run(goods.id);
    } else {
      db.prepare("UPDATE shop_goods SET exchange_count=exchange_count+1,updated_at=datetime('now') WHERE id=?").run(goods.id);
    }
  })();

  const updated = users.findById.get(user.id);
  res.json({ success: true, remain: updated.points });
});
...
// ── 修改签到接口：读取配置积分 ──
// 覆盖原 /user/checkin（原接口固定 10 分，现改为读配置）
router.post('/user/checkin/v2', requireAuth, (req, res) => {
  ...
  const pts = Math.max(1, parseInt(shopCfg('checkin_points') || '10'));
  users.addPoints.run(pts, user.id);
  points.insert.run(uuidv4(), user.id, pts, '每日签到');
  const updated = users.findById.get(user.id);
  res.json({ success: true, points: pts, streak: updated.checkin_streak, total: updated.points });
...
router.post('/admin/shop/config', requireAdmin(2), (req, res) => {
  const { checkin_points, redeem_code_on, kyc_cost_type, kyc_cost_value, kyc_feature_key } = req.body;
  const updates = {};
  if (checkin_points !== undefined) updates['checkin_points']  = String(Math.max(1, parseInt(checkin_points) || 10));
...
router.post('/shop/transfer', requireAuth, async (req, res) => {
  const { to_uid, amount, note } = req.body;
  const pts = parseInt(amount);
  if (!to_uid) return res.status(400).json({ error: '请指定收款用户' });
  ...
  // 查找收款用户
  const toUser = db.prepare('SELECT * FROM users WHERE uid_seq=? OR id=?').get(to_uid, to_uid);
  if (!toUser) return res.status(404).json({ error: '收款用户不存在' });
  if (toUser.id === req.user.uid) return res.status(400).json({ error: '不能给自己转账' });
  ...
  db.transaction(() => {
    ...
    points.insert.run(uuidv4(), fromUser.id, -pts, `转账给 ${toUser.name}（UID#${String(toUser.uid_seq).padStart(5,'0')}）`);
    points.insert.run(uuidv4(), toUser.id,   pts, `收到来自 ${fromUser.name} 的转账`);
  })();
...
router.get('/shop/find-user', requireAuth, (req, res) => {
  ...
  const showUid = shopCfg('transfer_show_uid') !== '0';

  if (!showUid) {
    // 隐私模式：uid 和 name 都必须填且匹配
    if (!uid || !name) return res.status(400).json({ error: '请同时输入 UID 和用户名' });
    const u = db.prepare('SELECT id,uid_seq,name,status FROM users WHERE (uid_seq=? OR id=?) AND name=?').get(uid, uid, name.trim());
    if (!u) return res.status(404).json({ error: '未找到匹配用户，请确认 UID 与用户名是否一致' });
    return res.json({ success: true, user: { uid_seq: u.uid_seq, name: u.name, status: u.status } });
  }

  // 公开模式：uid 查找即可
  if (!uid) return res.status(400).json({ error: '请输入 UID' });
  const u = db.prepare('SELECT id,uid_seq,name,status FROM users WHERE uid_seq=? OR id=?').get(uid, uid);
