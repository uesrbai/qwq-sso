...
// ── 启动时：把数据库中保存的环境变量注入 process.env ──
// 数据库中的值优先级高于 .env 文件（允许在管理端覆盖）
function loadEnvFromDb() {
  try {
    const rows = db.prepare('SELECT key_name, value FROM env_config WHERE value IS NOT NULL AND value != \'\'').all();
    let count = 0;
    ...
    if (count > 0) console.log(`[ENV] 从数据库加载了 ${count} 个环境变量`);
  } catch (e) {
    console.warn('[ENV] 数据库环境变量加载失败（可能是首次启动）:', e.message);
  }
}
loadEnvFromDb();
