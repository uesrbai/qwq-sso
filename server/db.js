...
// 三方登录「多主体」：一个渠道（如微信）可挂多个登录主体（多组织/多架构）。
// 环境变量里配的那套仍是各平台的「默认主体」（provider=平台名，零迁移）；
// 这张表存的是额外主体，provider=平台名:实例id。config 是该主体的凭证 JSON（含 secret）。
try { db.exec(`CREATE TABLE IF NOT EXISTS oauth_providers (
  id          TEXT PRIMARY KEY,
  platform    TEXT NOT NULL,                 -- wechat | wecom | feishu | ...
  label       TEXT NOT NULL DEFAULT '',      -- 主体名，如「A公司微信」，展示用
  config      TEXT NOT NULL DEFAULT '{}',    -- JSON：该主体的凭证（键名与该平台环境变量同名）
...
const oauthProviderStmts = {
  ...
  enabledByPlatform: db.prepare('SELECT * FROM oauth_providers WHERE platform=? AND enabled=1 ORDER BY sort_weight, created_at'),
  get:          db.prepare('SELECT * FROM oauth_providers WHERE id=?'),
  insert:       db.prepare('INSERT INTO oauth_providers (id,platform,label,config,enabled,sort_weight) VALUES (?,?,?,?,?,?)'),
