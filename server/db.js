...
const _insertUser = db.prepare(`INSERT INTO users
  (id,uid_seq,uid_code,name,email,phone,password_hash,role,admin_level,user_level,status)
  VALUES (@id,@uid_seq,@uid_code,@name,@email,@phone,@password_hash,@role,@admin_level,@user_level,@status)`);
...
// ──────────────────────────────────────────
const groupStmts = {
  all:    db.prepare('SELECT g.*, (SELECT COUNT(*) FROM users u WHERE u.group_id=g.id) AS user_count FROM user_groups g ORDER BY g.created_at ASC'),
...
const appStmts = {
  ...
  insert: db.prepare(`INSERT INTO apps (id,name,icon,icon_bg,description,client_id,client_secret,callback_url,launch_url,status,visible)
    VALUES (@id,@name,@icon,@icon_bg,@description,@client_id,@client_secret,@callback_url,@launch_url,@status,@visible)`),
  update: db.prepare(`UPDATE apps SET name=@name,icon=@icon,icon_bg=@icon_bg,description=@description,
    callback_url=@callback_url,launch_url=@launch_url,status=@status,visible=@visible,updated_at=datetime('now') WHERE id=@id`),
