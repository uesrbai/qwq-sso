...
const { db, nextUidSeq, users, oauth, oauthProviders, oauthSubjects, orgMembers, appOrgs, appVisibleToUser, otp, logs, apps, idp, twofa, webauthn, announcements, documents, apiKeys, env, points } = require('./db');
...
router.get('/admin/orgs/:sid/members', requireAdmin(3), (req, res) => {
  const s = oauthSubjects.get.get(req.params.sid);
  if (!s) return res.status(404).json({ error: '组织不存在' });
  res.json({ success: true, org: { id: s.id, name: s.name, uid_prefix: s.uid_prefix || '', uid_len: s.uid_len || 4 }, members: orgMembers.listBySubject.all(s.id) });
});

// 加成员：account 支持邮箱/手机/UID/用户名；org_uid 可指定，留空则按规则自动生成
router.post('/admin/orgs/:sid/members', requireAdmin(2), (req, res) => {
  const s = oauthSubjects.get.get(req.params.sid);
  if (!s) return res.status(404).json({ error: '组织不存在' });
...
router.patch('/admin/orgs/:sid/members/:uid', requireAdmin(2), (req, res) => {
  const s = oauthSubjects.get.get(req.params.sid);
  if (!s) return res.status(404).json({ error: '组织不存在' });
...
router.delete('/admin/orgs/:sid/members/:uid', requireAdmin(2), (req, res) => {
  const target = findRealUserByUid(req.params.uid);
...
// 组织内 UID 自动生成规则
router.patch('/admin/orgs/:sid/uid-rule', requireAdmin(2), (req, res) => {
  const s = oauthSubjects.get.get(req.params.sid);
  if (!s) return res.status(404).json({ error: '组织不存在' });
...
router.get('/apps/market', requireAuth, (req, res) => {
  ...
  const list = apps.findEnabled.all().filter(a => appVisibleToUser(a.id, req.user.uid));
...
router.post('/v1/users/:uid/kyc/session', requireApiKey('kyc:session'), async (req, res) => {
  ...
  try {
    ...
    const { result } = await createKycSession(u.id, callbackUrl, { name, idNumber });
