...
router.get('/v1/users', requireApiKey('users:read'), (req, res) => {
  ...
  // 公共账号（is_public=1）是共享身份、不是自然人，开放 API 一律排除，避免被当成用户同步过去
  let rows = status
    ? db.prepare('SELECT * FROM users WHERE status=? AND is_public=0 ORDER BY uid_seq LIMIT ? OFFSET ?').all(status,lim,off)
    : db.prepare('SELECT * FROM users WHERE is_public=0 ORDER BY uid_seq LIMIT ? OFFSET ?').all(lim,off);
