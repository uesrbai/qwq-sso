...
// ── 用户信息 ──
// ── KYC 实名认证接口 ──
const { createKycSession, verifyKycDirect, verifyDiditWebhook, verifyStripeWebhook, queryAlipayCertify } = require('./kyc');

// 支付宝实人认证待确认记录：发起时存 certify_id + 姓名/尾号，用户核身回跳后查询落库
try {
...
const kycPending = {
  set:    db.prepare(`INSERT INTO kyc_pending (user_id,provider,certify_id,name,id_tail) VALUES (?,?,?,?,?)
                      ON CONFLICT(user_id) DO UPDATE SET provider=excluded.provider,certify_id=excluded.certify_id,name=excluded.name,id_tail=excluded.id_tail,created_at=datetime('now')`),
...
router.post('/user/kyc/session', requireAuth, noPublic, async (req, res) => {
  ...
  try {
    ...
    // 支付宝：结果不随回跳带回，需在 callback 里用 certify_id 查询，先把待确认信息存下
    if (result.provider === 'alipay') {
      kycPending.set.run(user.id, 'alipay', result.session_id, name, idNumber.slice(-4));
    }
  ...
  if (pending && pending.provider === 'alipay') {
    try {
      ...
      if (passed) {
        ...
        if (user && !user.kyc_verified) {
          const nm = pending.name || '';
          const maskedName = nm.length <= 2 ? (nm[0] || '—') + '*' : nm[0] + '*'.repeat(nm.length - 2) + nm.slice(-1);
          db.prepare("UPDATE users SET kyc_verified=1, kyc_name=?, kyc_id_tail=?, kyc_provider=?, updated_at=datetime('now') WHERE id=?")
            .run(maskedName, pending.id_tail || '—', '支付宝实人认证', user_id);
        }
        success = true;
      }
...
router.post('/user/checkin/v2', requireAuth, noPublic, (req, res) => {
  ...
  // 连续签到：last_checkin 存的是完整 ISO 时间戳，比较时要取日期部分（否则字符串永远不等于 yesterday，连签永远断）
  const yesterday = new Date(now - 86400000).toISOString().slice(0,10);
  const lastDay   = lastCheckin ? lastCheckin.toISOString().slice(0,10) : null;
  if (lastDay === yesterday) users.checkin.run(user.id);   // 昨天签过 → 连续 +1
  else users.resetStreak.run(user.id);                     // 断签或首签 → 重置为 1
