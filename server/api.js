...
// 某分组下的公共账号列表
router.get('/admin/groups/:gid/public-accounts', requireAdmin(3), (req, res) => {
...
router.post('/admin/groups/:gid/public-accounts', requireAdmin(2), (req, res) => {
...
// 公共账号详情 + 授权成员
router.get('/admin/public-accounts/:id', requireAdmin(3), (req, res) => {
...
router.put('/admin/public-accounts/:id/members', requireAdmin(2), (req, res) => {
  const p = publicAccounts.get.get(req.params.id);
  if (!p) return res.status(404).json({ error: '公共账号不存在' });
...
router.delete('/admin/public-accounts/:id', requireAdmin(2), (req, res) => {
  const p = publicAccounts.get.get(req.params.id);
  if (!p) return res.status(404).json({ error: '公共账号不存在' });
  publicAccounts.clearMembers.run(p.id);
...
// 切换到公共账号：校验授权 + 仍在分组内 → 签发公共账号的独立令牌（前端另开会话用）
